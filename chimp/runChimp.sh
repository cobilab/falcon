...
if [[ "$GET_FALCON" -eq "1" ]]; then
  ...
  cp falcon/scripts/GetChimpParse.sh .
fi
#==============================================================================
# GET GOOSE
...
if [[ "$RUN_FALCON" -eq "1" ]]; then
  (time ./FALCON -v -n 8 -t 12000 -F -Z -m 20:100:1:5/10 -c 200 -y complexity.pt PT DB.fa ) &> REPORT-FALCON-PT ;
  (time ./FALCON-FILTER -v -F -sl 0.001 -du 20000000 -t 0.5 -o positions.pt complexity.pt ) &> REPORT-FALCON-FILTER-PT ;
  (time ./FALCON-EYE -v -e 500 -s 4 -F -o draw.svg positions.pt ) &> REPORT-FALCON-EYE-PT ;
fi
#==============================================================================
# RUN FILTER
