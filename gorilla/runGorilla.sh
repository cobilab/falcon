...
if [[ "$GET_FALCON" -eq "1" ]]; then
  ...
  cp falcon/scripts/GetGorillaParse.sh .
fi
#==============================================================================
# GET GOOSE
...
if [[ "$RUN_FALCON" -eq "1" ]]; then
  (time ./FALCON -v -n 8 -t 12000 -F -Z -m 20:100:1:5/10 -c 200 -y complexity.gg PT DB.fa ) &> REPORT-FALCON-GG ;
  (time ./FALCON-FILTER -v -F -sl 0.001 -du 20000000 -t 0.5 -o positions.gg complexity.gg ) &> REPORT-FALCON-FILTER-GG ;
  (time ./FALCON-EYE -v -e 500 -s 4 -F -o draw.svg positions.gg ) &> REPORT-FALCON-EYE-GG ;
fi
#==============================================================================
# RUN FILTER
